...
* **Опціональні query параметри:**
  ...
  - `week_start` *(дата `YYYY-MM-DD`, за замовчуванням — понеділок поточного тижня)* — понеділок тижня, на який записати план: поточного або наступного (так у неділю можна спланувати закупи на тиждень уперед). Саме дата, а не «наступний»: сторінка, відкрита в неділю й відправлена в понеділок, інакше записала б план на тиждень пізніше. Не понеділок, тиждень, що вже минув, або далі наступного → `400` з поясненням у `error`.

> 🌟 **Повна автоматична інтеграція:**
> - Усі параметри (бюджет, тренування, алергени, стоп-продукти, розклад) беруться з `user_settings`!
> - Останній фідбек користувача (оцінки страв, вибрані теги, а якщо фідбек прив'язаний до плану — витрати й зміна ваги за той тиждень) **автоматично враховується** агентом при формуванні нового меню! Попередній план агенту окремо не передається.
...
> Рахується автоматично відносно тижня, на який записується план
> (`week_start` у `/plan/stream`): ще один план на той самий тиждень отримує той
> самий номер; план на тиждень одразу після попереднього — номер +1; якщо
> між ними пропущено тиждень чи більше — рахунок починається заново з 1.
> Плани на пізніші тижні на номер не впливають. `week_start_date` — понеділок
> (UTC) тижня, до якого належить план; для плану на наступний тиждень він у майбутньому.
