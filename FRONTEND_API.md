...
## 4. Щотижневий Фідбек («Оцінка страв» та «Що агент змінить») 📝

Реалізує функціонал екрану щотижневого відгуку:
- Оцінка страв (`good` / `neutral` / `bad`)
- Швидкі теги зауважень
- Автоматична генерація бейджів рішень агента (`ВИЛУЧЕНО`, `СПРОЩЕНО`, `СІЛЬПО`, `КАЛОРІЇ`, `БІЛОК`, `СНЕКИ`)
...
```json
{
  ...
  "summary": "На основі ваших оцінок та тегу «Занадто складно готувати» алгоритм оновив параметри генерації:",
  "decisions": [
    {
      "badge": "ВИЛУЧЕНО",
      "badgeColor": "bg-[#FF5C00] text-white",
      "text": "Форель запечена з броколі та лимоном прибрано з раціону на наступні 3 тижні"
    },
    {
      "badge": "СПРОЩЕНО",
...
### 🔹 2. Попередній перегляд рішень агента в реальному часі (Preview)
Дозволяє фронтенду динамічно оновлювати блок «Що агент змінить» (`AgentDecisionWidget`) прямо під час кліків по тегах або оцінках страв, ще до відправки форми!
* **POST** `/feedbacks/preview`
* **Headers:** `Authorization: Bearer <token>`
* **Body:** такий самий (`dish_ratings` + `tags`).
* **Response (200 OK):** повертає `summary` та `decisions`.

---

### 🔹 3. Отримати останній збережений фідбек
* **GET** `/feedbacks/latest`
* **Headers:** `Authorization: Bearer <token>`
* **Response (200 OK):** останній об'єкт фідбеку з оцінками і рішеннями.
