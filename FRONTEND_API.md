...
* **Опціональні query параметри:**
  ...
  - `week` *(`current` | `next`, за замовчуванням `current`)* — на який тиждень записати план. `next` — тиждень, що починається з наступного понеділка: так у неділю можна спланувати закупи на тиждень уперед. Будь-яке інше значення → `400`.
...
> (`week` у `/plan/stream`): ще один план на той самий тиждень отримує той
...
> (UTC) тижня, до якого належить план; для `week=next` він у майбутньому.
